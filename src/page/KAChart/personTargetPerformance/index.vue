<template>
  ...
  <section class='query'>
      <Form :model='query' :label-width='60' inline>
        <FormItem label='开始月份' style='width: 180px;' >
          <DatePicker size='small' transfer v-model='query.beginYearMonth'  type='month' placeholder="选择年月" ></DatePicker>
        </FormItem>
        <FormItem label='结束月份' style='width: 180px;' >
          <DatePicker size='small'  transfer v-model='query.endYearMonth'  type='month' placeholder="选择年月" ></DatePicker>
        </FormItem>
        <FormItem label='办事处' style='width: 180px;'>
           <Select v-model='query.orgNo' transfer size='small' @on-change='getDealerNoList(query.orgNo)'>
             <Option v-for='item in orgList' :value='item.orgNo'  :key='item.orgNo' :label='item.orgName'></Option>{{}}
           </Select>
        </FormItem>
         <FormItem label='经销商' v-if='query.orgNo' style='width: 180px;' >
            <Select v-model='query.dealerNo' transfer size='small' @on-change='getMarketNoList(query.dealerNo)'>
              <Option v-for='item in dealerList' :value='item.dealerNo' :key='item.dealerNo' :label='item.dealerName'></Option>{{}}
            </Select>
        </FormItem>
        <FormItem label='零售商' v-if='query.dealerNo'  style='width: 180px;'>
            <Select v-model='query.marketNo' transfer size='small' @on-change='getStoreNoList(query.marketNo)'>
              <Option v-for='item in marketList' :value='item.marketNo' :key='item.marketNo' :label='item.marketName'></Option>{{}}
            </Select>
        </FormItem>
        <FormItem label='门店' v-if='query.marketNo' style='width: 180px;'>
  ...
  methods: {
    ...
    search(){
      ...
      if(params.beginYearMonth){
        ...
        params.beginYearMonth = begin.getFullYear() + '-' + (begin.getMonth()+1);
      }
